...
namespace mc
{
    ...
    namespace
    {
        ...
        // 读取网格全部 UV 层（TEXCOORD_0/1/2...），而不只是第一层。
        // 法线贴图等常引用第二套 UV（uvSet=1），若只读第一层会导致该 UV 在 mc::Mesh 中
        // 完全缺失，material 里的 texCoord 索引就会指向一个不存在的 UV 通道。
        inline std::vector<FbxGeometryElementUV *> CollectUVElements(FbxMesh *fbxMesh)
        {
            std::vector<FbxGeometryElementUV *> elems;
            int count = fbxMesh->GetElementUVCount();
            elems.reserve(count);
            for (int i = 0; i < count; ++i)
                elems.push_back(fbxMesh->GetElementUV(i));
            return elems;
        }
        ...
        struct VertexKey
        {
            ...
            std::vector<uint32_t> uvBits; // 每套 UV 的 (u, v) float → uint32 bit 模式，按 UV 层序号排列
        ...
        inline VertexKey MakeVertexKey(int ctrlIdx,
                                       ...
                                       const std::vector<Vec2> &uvValues)
        {
            VertexKey k;
            k.ctrlIdx = ctrlIdx;
            ...
            k.uvBits.resize(uvValues.size() * 2);
            for (size_t i = 0; i < uvValues.size(); ++i)
            {
                std::memcpy(&k.uvBits[i * 2], &uvValues[i].x, 4);
                std::memcpy(&k.uvBits[i * 2 + 1], &uvValues[i].y, 4);
            }
            return k;
        }
    ...
    {
        if (!fbxMesh)
            return {false, "FbxMeshHelper: null mesh"};
        ...
        std::vector<FbxGeometryElementUV *> uvElems = CollectUVElements(fbxMesh);
        outMesh.uvs.resize(uvElems.size());
        ...
        for (int p = 0; p < polyCount; ++p)
        {
            ...
            auto emitVertex = [&](int localIdx)
            {
                ...
                std::vector<Vec2> uvValues(uvElems.size());
                for (size_t i = 0; i < uvElems.size(); ++i)
                {
                    FbxVector2 uv = ReadUV(uvElems[i], ctrlIdx, vertexId);
                    uvValues[i] = {(float)uv[0], (float)(1.0 - uv[1])};
                }

                VertexKey key = MakeVertexKey(ctrlIdx, nxf, nyf, nzf, uvValues);
                ...
                else
                {
                    ...
                    for (size_t i = 0; i < uvValues.size(); ++i)
                        outMesh.uvs[i].push_back(uvValues[i]);
