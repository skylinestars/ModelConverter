...
namespace mc
{
    ...
    {
        if (std::abs(scaleToCm - 100.0) < 1e-6)
            return "m";
        if (std::abs(scaleToCm - 1.0) < 1e-6)
            return "cm";
        if (std::abs(scaleToCm - 0.1) < 1e-6)
            return "mm";
        if (std::abs(scaleToCm - 2.54) < 1e-6)
            return "inch";
        return "custom";
    }
    ...
    ObjectID FbxSceneConverter::ConvertMesh(FbxNode *fbxNode, Scene &mcScene)
    {
        ...
        std::map<int, std::vector<uint32_t>> perMatIndices;
        {
            ...
            for (int p = 0; p < polyCount2; ++p)
            {
                ...
                if (polySize < 3)
                    continue; // 与 FanTriangulateMesh 保持相同跳过条件
            ...
            for (int ci = 0; ci < channelCount; ++ci)
            {
                ...
                if (sparseCount > 0)
                {
                    ...
                    for (int si = 0; si < sparseCount; ++si)
                    {
                        ...
                        if (ctrlIdx < 0 || ctrlIdx >= (int)ctoMap.size())
                            continue;
                                                       ...
                                                       AnimationClip &clip)
    {
        ...
        for (auto &mcMesh : mcScene.meshes)
        {
            ...
            if (meshNodeIt == m_meshNodeMap.end())
                continue;
    ...
    // ============================================================
    FbxSkin *FbxSceneConverter::GetPrimarySkin(FbxMesh *fbxMesh) const
    {
        if (!fbxMesh)
            return nullptr;
        ...
        return FbxCast<FbxSkin>(fbxMesh->GetDeformer(0, FbxDeformer::eSkin));
    }

    void FbxSceneConverter::BuildSkeletonBones(FbxSkin *skin,
                                               FbxNode *fbxNode,
                                               Scene &mcScene,
                                               Skeleton &skeleton,
                                               std::unordered_map<FbxNode *, int> &boneIdxMap)
    {
        ...
        const FbxAMatrix meshGeo = GetGeometricTransform(fbxNode);
        ...
        for (int ci = 0; ci < clusterCount; ++ci)
        {
            ...
            FbxAMatrix ibp;
            cluster->GetTransformLinkMatrix(ibp);
            FbxAMatrix geoMatrix;
            cluster->GetTransformMatrix(geoMatrix);
            FbxAMatrix invBind = ibp.Inverse() * (geoMatrix * meshGeo.Inverse());

            bone.inverseBindPose = ToMatrix4(invBind);

            boneIdxMap[link] = (int)skeleton.bones.size();
            skeleton.bones.push_back(std::move(bone));
        }
    }

    void FbxSceneConverter::BuildBoneHierarchy(FbxSkin *skin,
                                               const std::unordered_map<FbxNode *, int> &boneIdxMap,
                                               Skeleton &skeleton)
    {
        if (!skin)
            return;
        ...
        for (int ci = 0; ci < clusterCount; ++ci)
        {
            ...
            auto parentIt = boneIdxMap.find(parentLink);
            auto childIt = boneIdxMap.find(link);
            if (parentIt != boneIdxMap.end() && childIt != boneIdxMap.end())
                skeleton.bones[childIt->second].parentBoneId = skeleton.bones[parentIt->second].id;
        }
    }

    bool FbxSceneConverter::ApplySkinWeights(mc::ObjectID meshId,
                                             FbxSkin *skin,
                                             const std::unordered_map<FbxNode *, int> &boneIdxMap,
                                             Scene &mcScene)
    {
        ...
        if (!mcMesh || mcMesh->positions.empty())
            return false;

        mcMesh->skinInfluences.resize(mcMesh->positions.size());

        const std::vector<std::vector<uint32_t>> *ctrlToOutput = nullptr;
        auto ctrlMapIt = m_ctrlToOutputMaps.find(meshId);
        if (ctrlMapIt != m_ctrlToOutputMaps.end())
            ctrlToOutput = &ctrlMapIt->second;
        ...
        for (int ci = 0; ci < clusterCount; ++ci)
        {
            ...
            for (int k = 0; k < idxCount; ++k)
            {
                ...
                bool hasCtrlMapping =
                    ctrlToOutput &&
                    ctrlIdx >= 0 &&
                    ctrlIdx < (int)ctrlToOutput->size() &&
                    !(*ctrlToOutput)[ctrlIdx].empty();
    ...
    size_t FbxSceneConverter::FixLeafBoneNodes(const std::unordered_map<FbxNode *, int> &boneIdxMap,
                                               Scene &mcScene)
    {
    ...
    void FbxSceneConverter::ConvertSkeletonForMesh(mc::ObjectID meshId,
                                                   FbxNode *fbxNode,
                                                   Scene &mcScene)
    {
        if (!fbxNode)
        ...
        FbxMesh *fbxMesh = fbxNode->GetMesh();
        FbxSkin *skin = GetPrimarySkin(fbxMesh);
        if (!skin)
            return;
        ...
        Skeleton skeleton;
        skeleton.id = mcScene.AllocateId();
        skeleton.name = fbxMesh->GetNode() ? fbxMesh->GetNode()->GetName() : "";

        Skin mcSkin;
        mcSkin.id = mcScene.AllocateId();
        mcSkin.meshId = meshId;
        mcSkin.skeletonId = skeleton.id;

        std::unordered_map<FbxNode *, int> boneIdxMap;
        BuildSkeletonBones(skin, fbxNode, mcScene, skeleton, boneIdxMap);
        BuildBoneHierarchy(skin, boneIdxMap, skeleton);

        mcScene.skeletons.push_back(std::move(skeleton));
        mcScene.skins.push_back(std::move(mcSkin));

        if (!ApplySkinWeights(meshId, skin, boneIdxMap, mcScene))
            return;

        size_t fixedLeafBoneCount = FixLeafBoneNodes(boneIdxMap, mcScene);
        if (fixedLeafBoneCount > 0)
        {
            Logger::Instance().LogInfo(
                "FbxSceneConverter: fixed " + std::to_string(fixedLeafBoneCount) +
                " leaf bone node(s) for mesh \"" + std::string(fbxNode->GetName()) + "\"");
        }
    }
    ...
    void FbxSceneConverter::ConvertSkeleton(FbxScene *fbxScene, Scene &mcScene)
    {
        ...
        for (auto &[meshId, fbxNode] : m_meshNodeMap)
            ConvertSkeletonForMesh(meshId, fbxNode, mcScene);
