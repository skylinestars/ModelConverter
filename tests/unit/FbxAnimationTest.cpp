// ============================================================
// Phase14 FBX 动画导入验收测试
// ============================================================
// 使用 maszyna_parowa.fbx（含动画的 FBX 文件）
//
// 验收标准：
//   - 动画导入成功
//   - 关键帧数量 > 0
...
// Fixture
// ============================================================
class FbxAnimationTest : public ::testing::Test
{
protected:
...
// ============================================================
// 导入 maszyna_parowa.fbx 并验证动画数据
// ============================================================

TEST_F(FbxAnimationTest, ImportMaszynaParowa_HasAnimations)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "maszyna_parowa.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;

    EXPECT_GT(scene.animations.size(), 0u);
}

TEST_F(FbxAnimationTest, ImportMaszynaParowa_AnimationHasValidDuration)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "maszyna_parowa.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;
...
TEST_F(FbxAnimationTest, ImportMaszynaParowa_AnimationHasKeyframes)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "maszyna_parowa.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;
...
TEST_F(FbxAnimationTest, ImportMaszynaParowa_ValidatePassPasses)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "maszyna_parowa.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;
...
TEST_F(FbxAnimationTest, ImportMaszynaParowa_AnimationNodeIdsValid)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "maszyna_parowa.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;
...
TEST_F(FbxAnimationTest, ImportMaszynaParowa_KeyframeTimesInRange)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "maszyna_parowa.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;
...
// ============================================================
// 导入无动画的 FBX（elephant3.fbx）验证不会崩溃
// ============================================================

TEST_F(FbxAnimationTest, ImportElephant3_NoAnimations_DoesNotCrash)
{
    mc::Scene scene;
    auto r = m_importer->Import(DataDir() + "elephant3.fbx", scene);
    ASSERT_TRUE(r.ok) << r.error;
