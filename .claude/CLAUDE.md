当前上下文使用中文交流。
# ModelConverter
这是一个类似 **Assimp + FBX Converter + USD Converter + Blender Import/Export Framework** 的通用三维格式转换平台

## 编码规范
- 默认使用C++17标准，优先使用标准库算法，杜绝C风格宏和裸new/delete。使用智能指针（std::unique_ptr、std::shared_ptr）管理内存。优先使用 RAII 模式。
- 在跨 DLL 接口和可序列化数据结构中，应尽量避免暴露 std::shared_ptr。内部实现优先使用 std::unique_ptr 表达所有权，跨对象引用使用裸指针、Handle 或 ID。这样可以减少 DLL ABI、序列化对象图以及生命周期管理带来的复杂性
- c++类不能超过1000行，c++函数不能超过300行，超过需要考虑拆分。
- 代码命名规范：
1. 类/类型：帕斯卡命名法（例如：Importer、aiScene）
2. 函数：帕斯卡命名法（例如：ReadFile、GetExtension）
3. 变量：驼峰命名法（例如：scene、importStep）
4. 常量：k 驼峰命名法或大写蛇形命名法（例如：kMaxVertices）
5. 成员变量：通常以 m_ 为前缀（例如：mScene）
6. 静态变量：通常以 s_ 为前缀
## 代码格式规范：
-使用UTF-8编码
-缩进：4个空格
